...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """   
    return SparseVector(numOHEFeats, [(OHEDict[feat], 1.0) for feat in rawFeats])
...
def parsePoint(point):
    ...
    """
    return [(idx-1, feat) for (idx, feat) in enumerate(point.split(',')) if idx > 0]
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    """
    features = [(idx-1, feat) for (idx, feat) in enumerate(point.split(','))]    
    encoding = oneHotEncoding(features[1:], OHEDict, numOHEFeats)
        
    return LabeledPoint(features[0][1], encoding)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(numOHEFeats, [(OHEDict[feat], 1.0) for feat in rawFeats if feat in OHEDict])
...
model0 = LogisticRegressionWithSGD.train(data=OHETrainData, iterations = numIters, 
                                     step = stepSize, miniBatchFraction = 1.0, initialWeights=None, regParam = regParam,
...
def getP(x, w, intercept):
    ...
    """
    rawPrediction = w.dot(x.features) + intercept
...
trainingPredictions = OHETrainData.map(lambda x:getP(x, model0.weights, model0.intercept))
...
def evaluateResults(model, data):
    ...
    logLoss = data.map(lambda x:computeLogLoss(getP(x, model.weights, model.intercept), x.label)).mean()
...
logLossValLR0 = OHEValidationData.map(lambda x:computeLogLoss(getP(x, model0.weights, model0.intercept), x.label)).mean()
...
labelsAndScores = OHEValidationData.map(lambda lp:
                                            (lp.label, getP(lp, model0.weights, model0.intercept)))
...
def parseHashPoint(point, numBuckets):
    ...
    """
    features = [(idx-1, feat) for (idx, feat) in enumerate(point.split(','))]    
    encoding = SparseVector(numBuckets, hashFunction(numBuckets, features[1:], False))
...
#xx = rawTrainData.take(1)
#xy = parseHashPoint(xx[0], numBucketsCTR)
#print xy
...
def computeSparsity(data, d, n):
    ...
    """    
    avgSparsity = data.map(lambda x: float(len(x.features.indices))).reduce(lambda x,y: x+y) / (d * n) 
