...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (x,y):(x-y)**2).mean())
...
firstModel = LinearRegressionWithSGD.train(data=parsedTrainData, iterations = numIters, 
                                     step = alpha, miniBatchFraction = miniBatchFrac, regParam =reg,
                                     regType = regType,
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, np.hstack((lp.features, map(lambda (x,y):x*y, itertools.product(lp.features, repeat=2)))))
